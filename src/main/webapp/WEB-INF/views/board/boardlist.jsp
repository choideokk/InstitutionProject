				...
				<div class="container">
					<div class="main-area">
						...
						<div class="main">
							...
							<div class="row">
								<form class="search_wrap" method="POST"
									action="${path}/boardlist?pageNo=1">
									<div class="search_area">
										<label> 대소문자 구분 <input type="checkbox"
											name="isChecked"
											<c:if
                                  test="${searchObj.isChecked == 'on'}">checked</c:if> />
										</label> <select name="searchKeyword">
											<option value="T"
												<c:if test="${searchObj.searchKeyword == 'T'}">selected</c:if>>제목
											</option>
											<option value="W"
												<c:if test="${searchObj.searchKeyword == 'W'}">selected</c:if>>작성자
											</option>
											<option value="TW"
												<c:if test="${searchObj.searchKeyword == 'TW'}">selected</c:if>>제목
												+ 작성자</option>
										</select> <input type="text" name="searchTxt"
											value="${searchObj.searchTxt}">
							...
							<c:if
								test='${searchObj.searchTxt eq "" ||  searchObj.searchTxt eq null}'>
								<ul class="pagination">
									...
									<li
										...
										<button
											data-path="${path}/${pagePath}?pageNo=${currentPageInfo.startPage-5}"
											aria-controls="dataTable"
											data-dt-idx="${currentPageInfo.startPage-5}" tabindex="0"
											class="page-link">Previous</button>
									</li>
									<!-- Previous 끝 -->
									<!-- Page번호 시작, 컨트롤러에서 list에 페이징 객체를 넣었으므로 list.변수명으로 불러옴 -->
									<c:forEach var="pNo" begin="${currentPageInfo.startPage}"
										end="${currentPageInfo.endPage}" step="1">
										<li
											class="paginate_button page-item <c:if test='${param.pageNo eq pNo}'>active</c:if>">
											<button data-path="${path}/${pagePath}?pageNo=${pNo}"
												aria-controls="dataTable" data-dt-idx="${pNo}" tabindex="0"
												class="page-link">${pNo}</button>
										</li>
									</c:forEach>
									<!-- Page번호 끝 -->
									<!-- Next 시작 -->
